...
func TestGetLockID(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "no lock returns empty string (NoSuchKey)",
			bucket:       "my-bucket",
			project:      "caas",
			stackRelPath: "workloads/dev/us-east-1/core/acm",
			region:       "us-east-1",
		...
		{
			name:         "no lock returns empty string (404 HeadObject)",
			bucket:       "my-bucket",
			project:      "caas",
			stackRelPath: "workloads/dev/us-east-1/core/acm",
			region:       "us-east-1",
			mockStderr:   "An error occurred (404) when calling the HeadObject operation: Not Found",
			mockExit:     1,
			wantID:       "",
		},
		{
